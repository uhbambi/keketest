...
END`,
    GET_CLOSEST_IMAGE: `CREATE PROCEDURE IF NOT EXISTS GET_CLOSEST_IMAGE(IN p_pHash CHAR(16)) READS SQL DATA
BEGIN
  DECLARE i_pHash BIGINT UNSIGNED;
  SET i_pHash = CONV(p_pHash, 16, 10);
  SELECT mid FROM ImageHashes WHERE BIT_COUNT(pHash ^ i_pHash) < 5 LIMIT 1;
END`,
    GET_CLOSEST_BANNED_IMAGE: `CREATE PROCEDURE IF NOT EXISTS GET_CLOSEST_BANNED_IMAGE(IN p_pHash CHAR(16)) READS SQL DATA
BEGIN
  DECLARE i_pHash BIGINT UNSIGNED;
  SET i_pHash = CONV(p_pHash, 16, 10);
